...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderFrame(bb.Min, bb.Max, ImColor(30, 30, 30), true, 11);
    static auto outline = [](ImGuiWindow* wnd, ImVec2 p_min, ImVec2 p_max, ImColor col, float r) -> void {
        wnd->DrawList->AddRect(p_min, p_max, col, r);
    };
    outline(window, bb.Min, bb.Max, ImColor(0, 0, 0), 11);
...
bool ImGui::ButtonExT(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::ButtonTransparent(const char* label, const ImVec2& size_arg)
{
    return ButtonExT(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), 34.f); // Arbitrary default of 8 lines high for multi-line
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(30, 30, 30), true, style.FrameRounding);
        static auto outline = [](ImGuiWindow* wnd, ImVec2 p_min, ImVec2 p_max, ImColor col, float r) -> void {
            wnd->DrawList->AddRect(p_min, p_max, col, r);
        };
        outline(window, frame_bb.Min, frame_bb.Max, ImColor(0, 0, 0), 0.f);
    }
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(40, 40, 40));
                }
