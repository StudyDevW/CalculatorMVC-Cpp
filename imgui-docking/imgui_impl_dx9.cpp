...
static void ImGui_ImplDX9_SetupRenderState(ImDrawData* draw_data)
{
    ...
    bd->pd3dDevice->SetVertexShader(vertexShader);
...
void ImGui_ImplDX9_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (!bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        if (bd->pd3dDevice->CreateVertexBuffer(bd->VertexBufferSize * sizeof(ImDrawVert), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, 0, D3DPOOL_DEFAULT, &bd->pVB, nullptr) < 0)
            return;
    }
    ...
    if (bd->pVB->Lock(0, (UINT)(draw_data->TotalVtxCount * sizeof(ImDrawVert)), (void**)&vtx_dst, D3DLOCK_DISCARD) < 0)
        return;
    ...
    bd->pd3dDevice->SetStreamSource(0, bd->pVB, 0, sizeof(ImDrawVert));
    bd->pd3dDevice->SetIndices(bd->pIB);
    bd->pd3dDevice->SetVertexDeclaration(vertexDeclaration);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                bd->pd3dDevice->SetTextureStageState(0, D3DTSS_COLOROP, texture == bd->FontTexture ? D3DTOP_SELECTARG2 : D3DTOP_MODULATE);
...
bool ImGui_ImplDX9_CreateDeviceObjects()
{
    ...
    if (!vertexDeclaration) {
        ...
        bd->pd3dDevice->CreateVertexDeclaration(elements, &vertexDeclaration);
    }

    if (!vertexShader)
        bd->pd3dDevice->CreateVertexShader(reinterpret_cast<const DWORD*>(default_vs), &vertexShader);
